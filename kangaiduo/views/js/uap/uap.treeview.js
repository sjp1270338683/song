uap.define("treeview", function($, exports, module) {
    ...
    var listRender = uap.view.template(listTmp);
    ...
    Treeview.prototype = {
        ...
        getContent : function(data, settings) {
            ...
            var index = 0;
            if (!settings.type) {
                if ((uap.isArray(contentData) || uap.isPlainObject(contentData)) && !$.zepto.isZ(contentData)) {
                    var warper = $('<div class="listview"></div>');
                    settings.option.selector = warper;
                    var lv = uap.listview(settings.option);
                    lv.set(contentData);
