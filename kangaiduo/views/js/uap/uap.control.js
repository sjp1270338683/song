 ...
 */
/*global uap,window */
uap.use("detect", function($, detect) {
    if (detect.os.ios) {
...
uap.extend(function(app, exports, module) {
    var $ = uap.require('dom');
    var appWin = uap.require('window');
    var locStorage = uap.require('locStorage');
    var view = uap.require('view');
    /*
     * 兼容pc，移动设备的封装
    ...
    function select(sel, cb) {
        ...
        $("select", $(sel)).on("change", function(evt) {
            ...
            uap.selectChange(evt.currentTarget);
            cb && cb(ele, ele.val());
        });
    }
    ...
    function touch(className, fun) {
        ...
        if (hasTouch) {
            ...
            $ele.on('tap', function() {
                if (uap.isFunction(ed.startCallFun)) {
